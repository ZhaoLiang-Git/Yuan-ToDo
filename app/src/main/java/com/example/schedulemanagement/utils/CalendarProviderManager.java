...
public class CalendarProviderManager {
    ...
    public static int addCalendarEvent(Context context, CalendarEvent calendarEvent) {
        ...
        setupEvent(calendarEvent, event,false);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (PackageManager.PERMISSION_GRANTED == context.checkSelfPermission(
                ...
                if (calendarEvent.getHasAlarm() == 1) {
                    ...
                    extendedProperties.put(CalendarContract.ExtendedProperties.EVENT_ID, ContentUris.parseId(eventUri));
        ...
        if (-2 != calendarEvent.getAdvanceTime()) {
            ...
            long eventIDs = Long.parseLong(reminderUri.getLastPathSegment());
            System.out.println("输出获取到的事件ID："+ eventID + "ids :"+eventIDs);
            if (null == reminderUri) {
                return -1;
            }
        }
    ...
    public static void deleteCalendarEvent(Context context,long begin, long end, String title){
        ...
        int id = cursor.getInt(cursor.getColumnIndex(CalendarContract.Calendars._ID));//取得id
        Uri deleteUri = ContentUris.withAppendedId(CalendarContract.Events.CONTENT_URI, id);
        int rows = context.getContentResolver().delete(deleteUri, null, null);
        if (rows == -1) {
            // 删除提醒失败直接返回
            System.out.println("删除提醒失败");
            return;
        }
    }
     ...
     * 组装日历事件
     */
    private static void setupEvent(CalendarEvent calendarEvent, ContentValues event,boolean isUpdata) {
        if (!isUpdata){
            //设置提醒id
            event.put(CalendarContract.Events._ID,  calendarEvent.getId());
        }
        // 事件开始时间
        ...
        if (null != calendarEvent.getRRule()) {
            // 设置事件重复规则
           // event.put(CalendarContract.Events.RRULE, getFullRRuleForRRule(calendarEvent.getRRule(), calendarEvent.getStart(), calendarEvent.getEnd()));
            event.put(CalendarContract.Events.RRULE, calendarEvent.getRRule());
        }
    }
...
//                return builder.append(rRule).append(getFinalRRuleMode(endTime)).toString();
//            case RRuleConstant.REPEAT_CYCLE_WEEKLY:
//                return builder.append(rRule).append(getWeekForDate(beginTime)).append("; UNTIL = ")
//                        .append(getFinalRRuleMode(endTime)).toString();
//            case RRuleConstant.REPEAT_CYCLE_MONTHLY:
//                return builder.append(rRule).append(getDayOfMonth(beginTime))
//                        .append("; UNTIL = ").append(getFinalRRuleMode(endTime)).toString();
//            default:
//                return rRule;
//        }
//    }
